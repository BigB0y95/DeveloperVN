...
from Posts.models import Posts as post_model
...
def get_lesson_by_id(request, course_id, subject_id, lesson_id, url_name):
    ...
    # get list post new
    post_list = post_model.objects.all().order_by('createDate')[0:4]
    ...
    if(len(lessons_next) > 0):
        ...
        if(len(lessons_prev) > 0):
            ...
            'lesson_next': lesson_next, 'lesson_prev':lesson_prev, 'info_account_list': info_account_list, 'logo':logo, 'post_list':post_list})
        else:
            disable_prev = "disabled"
            return render(request, 'pages/lesson.html', {'course_list' : course_list, 'subject' : subject,
            'lesson' : lesson, 'lesson_list' : lesson_list, 'disable_next': disable_next, 'disable_prev': disable_prev,
            'lesson_next': lesson_next, 'info_account_list':info_account_list, 'logo':logo, 'post_list':post_list})
    else:
        ...
        if(len(lessons_prev) > 0):
            ...
            return render(request, 'pages/lesson.html', {'course_list' : course_list, 'subject' : subject,
            'lesson' : lesson, 'lesson_list' : lesson_list, 'disable_next': disable_next, 'disable_prev': disable_prev,
            'lesson_prev':lesson_prev,'info_account_list':info_account_list, 'logo':logo, 'post_list':post_list})
        else:
            disable_prev = "disabled"
            return render(request, 'pages/lesson.html', {'course_list' : course_list, 'subject' : subject,
            'lesson' : lesson, 'lesson_list' : lesson_list, 'disable_next': disable_next,'info_account_list':info_account_list, 'logo':logo, 'post_list':post_list})